...
const StoreDashboard = ({ marketName }) => {
  ...
  const fetchMarketData = async () => {
    ...
    try {
      const currentMarket =
        marketName || atob(localStorage.getItem("marketName"));
      const url = new URL(
        `${process.env.REACT_APP_BASE_URL}/getstorewiseuploadcount`
      );
      ...
      const params = new URLSearchParams({
        ...
        ...(selectedFilters.markets.length && {
          markets: selectedFilters.markets.join(","),
        }),
        ...(selectedFilters.stores.length && {
          stores: selectedFilters.stores.join(","),
        }),
      });
